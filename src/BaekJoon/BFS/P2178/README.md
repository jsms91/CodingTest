## 미로 탐색(실버1) - 백준(2178번-BFS)
```
*** 문제 풀이 ***
DFS보다 BFS가 탐색이 이 문제에 더 적합한 이유는 BFS는 해당 깊이에서 갈 수 있는 노드 탐색을
마친 후 다음 깊이로 넘어간다.
즉, 찾는 순간 바로 탐색으로 결과를 찾을 수 있기 때문에 DFS보다 적합하다.
물론 DFS 방식으로 문제를 해결해도 되지만 DFS 방식은 다시 뒤로 돌아가서 다시 찾는 과정을 거쳐야 한다.
```
